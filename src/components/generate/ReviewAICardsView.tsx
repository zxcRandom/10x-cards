...
export default function ReviewAICardsView({ deckId }: ReviewAICardsViewProps) {
  ...
  const handleDiscard = (cardId: string) => {
    setCards(prev =>
      prev.map(card =>
        card.id === cardId ? { ...card, discarded: true, selected: false } : card
      )
    );
  ...
  const handleKeepAll = () => {
    setCards(prev =>
      prev.map(card =>
        card.discarded ? card : { ...card, selected: true }
      )
    );
  ...
  const handleUncheckAll = () => {
    setCards(prev =>
      prev.map(card => ({ ...card, selected: false }))
    );
  ...
  const canSave =
    ...
    (destination.mode === 'new'
      ? destination.newName.trim().length > 0
      : destination.existingDeckId !== null);
