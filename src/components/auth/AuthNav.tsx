...
export default function AuthNav({ userEmail, currentPath = "/" }: AuthNavProps) {
  ...
  const linkClass = (path: string) =>
    `text-sm font-medium transition-colors ${
      isActive(path) ? "text-foreground font-semibold" : "text-muted-foreground hover:text-foreground"
    }`;
      ...
      <div className="container max-w-6xl mx-auto px-4 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            ...
            <a href="/decks" className={linkClass("/decks")} aria-current={isActive("/decks") ? "page" : undefined}>
              Moje talie
            </a>
            <a
