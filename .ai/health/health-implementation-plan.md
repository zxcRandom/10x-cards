...
const { data, error } = await supabase.from("profiles").select("*").eq("id", "some-id");
```
...
export const GET: APIRoute = async ({ locals }) => {
  ...
  try {
    ...
    const { error: dbError } = await locals.supabase.from("profiles").select("id", { head: true }).limit(1);
...
describe("GET /api/v1/health", () => {
  ...
  it("should handle multiple concurrent requests", async () => {
    const requests = Array(10)
      .fill(null)
      .map(() => fetch(`${baseUrl}/api/v1/health`));
