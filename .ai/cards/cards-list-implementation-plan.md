...
const cardsListSchema = z.object({
  ...
  sort: z
    .enum(["createdAt", "updatedAt", "nextReviewDate", "easeFactor", "intervalDays", "repetitions"])
    .default("createdAt"),
...
export const GET: APIRoute = async ({ params, url, locals }) => {
  try {
    ...
    let query = locals.supabase.from("cards").select("*").eq("deck_id", validated.deckId);
    ...
    const sortField =
      validated.sort === "createdAt"
        ...
        : validated.sort === "updatedAt"
          ? "updated_at"
          : validated.sort === "nextReviewDate"
            ? "next_review_date"
            : validated.sort === "easeFactor"
              ? "ease_factor"
              : validated.sort === "intervalDays"
                ? "interval_days"
                : "repetitions";
