...
const CreateDeckSchema = z.object({
  name: z.string().trim().min(1, "Deck name is required").max(255, "Deck name must not exceed 255 characters"),
...
describe("DeckService.createDeck", () => {
  ...
  it("should throw error if database insert fails", async () => {
    ...
    await expect(service.createDeck("user-123", { name: "Test" })).rejects.toThrow("Failed to create deck");
  });
});
```
