...
```typescript
return new Response(JSON.stringify(reviewResponseDTO), {
  status: 200,
...
function calculateSM2Parameters(
  ...
  if (grade < 3) {
    ...
    newEaseFactor = Math.max(EASE_FACTOR_MIN, currentEaseFactor + EASE_FACTOR_INCREASE - (5 - grade) * GRADE_PENALTY);
  }
...
export const POST: APIRoute = async ({ params, request, locals }) => {
  try {
    ...
    const sm2Update = calculateSM2Parameters(card.ease_factor, card.interval_days, card.repetitions, validated.grade);
