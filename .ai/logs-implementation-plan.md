...
1. **Build query**:
   ...
   ```typescript
   let query = supabase.from("ai_generation_logs").select("*", { count: "exact" }).eq("user_id", userId);
...
// W query
const selectOptions = filters.includeTotal ? { count: "exact" } : {};

const { data, count } = await supabase.from("ai_generation_logs").select("*", selectOptions);
// ...
 ...
 */
export const listAILogsQuerySchema = z
  .object({
    // Filters
    deckId: z.string().uuid("Invalid deck ID format").optional(),

    from: z.string().datetime("Invalid from date format. Use ISO-8601.").optional(),

    to: z.string().datetime("Invalid to date format. Use ISO-8601.").optional(),
...
describe("listAILogsQuerySchema", () => {
  ...
  it("should reject invalid UUID", () => {
    expect(() => listAILogsQuerySchema.parse({ deckId: "not-a-uuid" })).toThrow("Invalid deck ID format");
  });
  ...
  it("should reject limit > 100", () => {
    expect(() => listAILogsQuerySchema.parse({ limit: 101 })).toThrow("Limit must not exceed 100");
  });
});
```
...
export class AILogService {
   ...
   */
  static async createLog(supabase: SupabaseClient, data: CreateAILogData): Promise<DbAILog> {
   ...
   */
  static async listLogs(supabase: SupabaseClient, userId: string, options: ListLogsOptions): Promise<AILogsListDTO> {
   ...
   */
  static async getLogById(supabase: SupabaseClient, logId: string, userId: string): Promise<DbAILog | null> {
...
describe("GET /api/v1/ai/logs", () => {
  ...
  it("should filter by deckId", async () => {
    const response = await fetch(`http://localhost:4321/api/v1/ai/logs?deckId=${testDeckId}`, {
  ...
  it("should filter by date range", async () => {
    ...
    const response = await fetch(`http://localhost:4321/api/v1/ai/logs?from=${from}&to=${to}`, {
      headers: { Authorization: `Bearer ${authToken}` },
    });
  ...
  it("should respect pagination", async () => {
    const response = await fetch("http://localhost:4321/api/v1/ai/logs?limit=5&offset=0", {
      headers: { Authorization: `Bearer ${authToken}` },
    });
  ...
  it("should sort ascending", async () => {
    const response = await fetch("http://localhost:4321/api/v1/ai/logs?order=asc&limit=10", {
      headers: { Authorization: `Bearer ${authToken}` },
    });
  ...
  it("should reject invalid deckId", async () => {
    const response = await fetch("http://localhost:4321/api/v1/ai/logs?deckId=not-a-uuid", {
      headers: { Authorization: `Bearer ${authToken}` },
    });
